...
const {getAnnotatedScope, hasJSDocWithTags} = require('../ast-utils');
...
module.exports = {
	...
	create: (context) => {
		...
		function resolveMethodScope(methodDefinition) {
			const methodScope = getAnnotatedScope(methodDefinition, sourceCode);

			if (!methodScope) {
				const hasImplicitRestrictedScope = methodDefinition.key.name.startsWith('_');
				const isOverridden = hasJSDocWithTags(methodDefinition, ['override', 'inheritdoc'], sourceCode);

				if (hasImplicitRestrictedScope && isOverridden) {
		...
		function check(methodADefinition, methodBDefinition) {
			...
			const isMethodAOverridden = hasJSDocWithTags(methodADefinition, ['override', 'inheritdoc'], sourceCode);
			const isMethodBOverridden = hasJSDocWithTags(methodBDefinition, ['override', 'inheritdoc'], sourceCode);
