...
module.exports = {
	meta: {
		docs: {
			description: 'enforce consistent indentation in JSDoc'
		},
		schema: [{
			oneOf: [{
				enum: ['tab']
	...
	create: (context) => {
		...
		function addTopBracesIndent(tokens, type, indentRanges) {
			const typeTokens = getJSDocSubtypeTokens(tokens, type);
		...
		function addRecordIndent(tokens, type, indentRanges) {
			type.fields.forEach((field) => {
				...
				const valueTokens = getJSDocSubtypeTokens(tokens, field.value);
		...
		function addFunctionIndent(tokens, type, indentRanges) {
			type.params.forEach((param) => {
				...
				if (param.type === 'ParameterType') {
					const expressionTokens = getJSDocSubtypeTokens(paramTokens, param.expression);
			...
			if (type.this) {
				const thisTokens = getJSDocSubtypeTokens(tokens, type.this);
				const name = findFirstIdentifier(tokens, 'this');

				if (isLineBreakBefore(tokens, name)) {
					indentRanges.push([
						name.range[0],
			...
			if (type.result) {
				const resultTokens = getJSDocSubtypeTokens(tokens, type.result);
		...
		function check(JSDoc) {
			JSDoc.tags.forEach((tag) => {
				...
				traverseJSDocType(tag.type, (type) => {
					const tokens = getJSDocSubtypeTokens(tagTokens, type);
