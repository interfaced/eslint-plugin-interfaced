...
module.exports = {
	...
	create: (context) => {
		...
		function reportOrderViolationIfNeeded(methodADefinition, methodBDefinition) {
			...
			if (wrongOrderByConstructor) {
				context.report({
					...
					message: 'Constructor method should be the first in the class body.'
				});
			}
		...
		return {
			'MethodDefinition': (node) => methodDefinitions.push(node),
			'Program:exit': () => {
				methodDefinitions.reduce((previousMethodDefinition, methodDefinition) => {
					if (previousMethodDefinition) {
						reportOrderViolationIfNeeded(previousMethodDefinition, methodDefinition);
					}
