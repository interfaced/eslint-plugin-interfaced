...
module.exports = {
	...
	create: (context) => {
		...
		function isCollisionWithAnotherClassBefore(classNode) {
			...
			const classJDocComment = getJSDocComment(classNode, sourceCode);

			const nodeBeforeClass = classScopeIndex ? scopeBody[classScopeIndex - 1] : null;
			const tokenBeforeClass = sourceCode.getTokenBefore((classJDocComment || classNode), {
		...
		function isCollisionWithAnotherClassAfter(classNode) {
			...
			const nodeAfterClassJDocComment = nodeAfterClass && getJSDocComment(nodeAfterClass, sourceCode);

			return (
				tokenAfterClass &&
				nodeAfterClass && isClass(nodeAfterClass) &&
				(nodeAfterClassJDocComment || nodeAfterClass).loc.start.line === tokenAfterClass.loc.start.line
			);
		}
		...
		function check(classNode) {
			...
			if (tokenBeforeClass) {
				...
				const classStartLine = (classJDocComment || classNode).loc.start.line;
